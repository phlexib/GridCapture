...
@IBDesignable class ADVSegmentedControl: NSControl {
    
    private var labels = [NSTextField]()
    var thumbView = NSView()
    ...
    @IBInspectable var selectedLabelColor : NSColor = NSColor.blackColor() {
        didSet {
            setSelectedColors()
        }
    }
    
    @IBInspectable var unselectedLabelColor : NSColor = NSColor.whiteColor() {
        didSet {
            setSelectedColors()
        }
    }
    
    @IBInspectable var thumbColor : NSColor = NSColor.whiteColor() {
        didSet {
            setSelectedColors()
        }
    }
    
    @IBInspectable var borderColor : NSColor = NSColor.whiteColor() {
        didSet {
            layer?.borderColor = borderColor.CGColor
        }
    }
    
    @IBInspectable  var thefont : NSFont! = NSFont.systemFontOfSize(12) {
        didSet {
            setFont()
        }
    }
    ...
    func setupView(){
        ...
        layer?.borderColor = NSColor(white: 1.0, alpha: 0.5).CGColor
        layer?.borderWidth = 2
    ...
    override func mouseDown(theEvent: NSEvent) {
        ...
        let location = mouse(<#T##aPoint: NSPoint##NSPoint#>, inRect: <#T##NSRect#>).locationInView(self)
    ...
    func displayNewSelectedIndex(){
        ...
        NSView.animateWithDuration(0.5, delay: 0.0, usingSpringWithDamping: 0.5, initialSpringVelocity: 0.8, options: nil, animations: {
    ...
    func addIndividualItemConstraints(items: [NSView], mainView: NSView, padding: CGFloat) {
